...
#el cupo se cuenta SIN el jefe: el jefe no es relleno, es el motivo de su oleada, y no cuenta
#ni para la curva ni para el techo
def cupoDeTropa(numero):
    composicion = oleadas.composicion(numero)
    return sum(cuantos for tipo, cuantos in composicion.items() if tipo != oleadas.JEFE)


cupos = [cupoDeTropa(numero) for numero in range(1, 30)]
comprobar("el cupo de tropa nunca baja de una oleada a la siguiente",
...
comprobar("y el jefe va aparte del techo: uno, y solo en su oleada",
          all(oleadas.composicion(numero)[oleadas.JEFE] == (1 if oleadas.tocaJefe(numero) else 0)
              for numero in range(1, 40)),
          str([numero for numero in range(1, 40) if oleadas.tocaJefe(numero)]))
...
#la oleada 6 no trae jefe, asi que se compara con los tipos que de verdad trae su cupo
tiposDeLaSexta = set(tipo for tipo, cuantos in oleadas.composicion(6).items() if cuantos)
comprobar("saca todos los tipos que traia su cupo, y ninguno mas",
          set(sacados) == tiposDeLaSexta,
